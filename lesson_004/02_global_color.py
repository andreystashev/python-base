...
while True:
    ...
    color_input = int(value)  # TODO Не нужно к int привоить, у нас ключами словаря являются строки
    print('Вы ввели', value)

    # TODO Тут тогда можно проверить на вхождение в словарь через оператор in
    if color_input > 4 or color_input < 1:
        ...
        # TODO Мы для этого и делали словарь, чтобы не делать такой цепочки - юзер уже ввел номер, который
        #   у нас является ключом словаря
        if color_input == 1:
        ...
        break
#  Чтобы не прерывать цикл при неверном вводе пришлось запихнуть всё внутрь цикла,
...
#  то не срабатывает
# TODO Про вектор запись верная, просто в самой функции рисования фигуры нужно сделать входной аргумент -
#  цвет, и при вызове функций передавать туда нужный. Так что переносим сюда функции из  1 задания и
#  добавляем туда еще один входной аргумент. Соответственно, нужно их определить вверху модуля, не в цикле.
