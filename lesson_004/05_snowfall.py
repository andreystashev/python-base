...
# sd.random_number()
# sd.user_want_exit()
# TODO Нам нужны списки и для координат х и для координат у по N элементов каждом
...
#            и сдвиг координат
#
#  Не пойму, как реализовать смещение по игрику. Внутри цикла фор если изменять, то снежинка скачет после
...
while True:
    ...
    # TODO В цикле for мы будет проходится по этим спискам. То есть за каждую итерацию цикла for мы будем
    #   рисовать одну снежинку и сдвигать ее -
    #   завели цикл for i по range(N)
    #         нарисовали снежинку в x[i] и y[i] цветом фона
    #         уменьшили y[i] (сдвинули снежинку вниз)
    #         нарисовали снежинку в x[i] и y[i] белым цветом
    #         проверили, что если y[i] достигла низа - задали для нее новое значение (чтобы снежинка опять наверху оказалась)
